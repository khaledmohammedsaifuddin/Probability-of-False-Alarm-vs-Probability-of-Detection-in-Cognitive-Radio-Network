...
for p = 1:length(Pf)
    ...
    for kk=1:600 % No. of Monte Carlo Simulations
        %x=sin(2*pi*f*t);
        ...
        Signal = v(1:L);
        Rev_Sig1 = R.*Signal + n1; %rician
        Rev_Sig2 = R.*Signal + n2;
        Rev_Sig3 = R.*Signal + n3;
...
title('ROC CURVE UNDER RICIAN and NAKAGAMI Channel');
legend('rician for snr= 0db','nakagami for snr = 0db' ,'rician for snr= -1db','nakagami for snr = -1db', 'rician for snr= -2db','nakagami for snr = -2db');
