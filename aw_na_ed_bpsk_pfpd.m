...
for p = 1:length(Pf)
    ...
    for kk=1:60000 % No. of Monte Carlo Simulations
        %x=sin(2*pi*f*t);
        ...
        %rayleigh
        %h=1/sqrt(2)*(randn(1,L)+4i*randn(1,L));
        ...
        Signal = v(1:L);
        Rev_Sig1 = Signal + Noise1; %awgn
        Rev_Sig2 = Signal + Noise2;
        Rev_Sig3 = Signal + Noise3;
...
t2;
figure
semilogy(Pf,Pd2,'-k',Pf,Pd6,'-*k',Pf,Pd10,'->k','LineWidth', 3);
hold on
grid on
xlabel('Probability of false alarm,P_{f}','FontSize',30);
ylabel('Probability of detection,P_{d}','FontSize',30);
title('ROC Curve Under  Nakagami  Channel','FontSize',30);
leg=legend('nakagami for snr= -4dB', 'nakagami for snr= 0dB ',  'nakagami for snr= 2dB');
set(leg,'FontSize',32)