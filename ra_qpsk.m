...
for p = 1:length(Pf)
    ...
    for kk=1:40000 % No. of Monte Carlo Simulations
        ...
        h1=1/sqrt(2)*(randn(1,L)+4i*randn(1,L));  %Rayleigh Flat Fading - single tap
        ...
        Signal = v(1:L); %BPSK
        Recv_Sig1 = h1.*Signal + n1;
        Recv_Sig2 = h2.*Signal + n2;
        Recv_Sig3 = h3.*Signal + n3;
...
figure
loglog(Pf,pm1,'-b',Pf,pm2,'-r',Pf,pm3,'*-b',Pf,pm4,'*-r',Pf,pm5,'--b',Pf,pm6,'--r', 'LineWidth', 3);
hold on
grid on
xlabel('Probability of false alarm,P_{f}','FontSize',30);
ylabel('Probability of misdetection,P_{m}','FontSize',30);
title('CROC Curve Under Rayleigh Channel For BPSK and QPSK','FontSize',30);
leg=legend('BPSK for snr= -4dB','QPSK for snr= -4dB', 'BPSK for snr= 0dB','QPSK for snr= 0dB','BPSK for snr= 2dB','QPSK for snr= 2dB');
set(leg,'FontSize',32)