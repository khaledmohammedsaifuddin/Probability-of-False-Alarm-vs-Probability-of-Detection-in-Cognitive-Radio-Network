...
for p = 1:length(Pf)
    ...
    for kk=1:68000 % No. of Monte Carlo Simulations
        %x=sin(2*pi*f*t);
        ...
        Rec_Sig1 = R.*Signal + n1; %rician
        Rec_Sig2 = R.*Signal + n2;
        Rec_Sig3 = R.*Signal + n3;
        
        Recv_Sig1 = N.*Signal + n1; %nakagami
        Recv_Sig2 = N.*Signal + n2;
        Recv_Sig3 = N.*Signal + n3;
        ...
        Energy4 = abs(Rec_Sig1).^2;
        Energy5 = abs(Rec_Sig2).^2;
        Energy6 = abs(Rec_Sig3).^2;
        Energy7 = abs(Recv_Sig1).^2;
        Energy8 = abs(Recv_Sig2).^2;
        Energy9 = abs(Recv_Sig3).^2;
...
t2;
figure
loglog(Pf, pm1,'-b',Pf,pm2,'--b',Pf,pm3,'->b',Pf,pm5,'-r',Pf,pm6,'--r',Pf,pm7,'->r',Pf,pm9,'-c',Pf,pm10,'--c',Pf,pm11,'->c','LineWidth', 3);
hold on
grid on
xlabel('Probability of false alarm,P_{f}','FontSize',30);
ylabel('Probability of misdetection,P_{m}','FontSize',30);
title('CROC Curve Under Rayleigh, Rician and Nakagami Channel','FontSize',30);
leg=legend('rayleigh for snr= -4dB','rician for snr = -4dB','nakagami for snr = -4dB', 'rayleigh for snr= 0dB ','rician for snr = 0dB','nakagami for snr= 0 dB', 'rayleigh for snr= 2dB','rician for snr = 2dB','nakagami for snr= 2dB');
set(leg,'FontSize',20)